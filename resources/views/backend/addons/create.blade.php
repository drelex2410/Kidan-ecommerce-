...
<div class="col-lg-7 mx-auto">
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">{{ translate('Install or update Addon')}}</h5>
        </div>
        <form class="form-horizontal" action="{{ route('addons.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="card-body">
                <div class="form-group row">
                    <label class="col-sm-3 col-from-label" for="purchase_code">{{ translate('Reference code')}}</label>
                    <div class="col-sm-9">
                        <input type="text" id="purchase_code" name="purchase_code" class="form-control" autocomplete="off">
                        <small class="text-muted">{{ translate('Optional local reference for your records.') }}</small>
                    </div>
                </div>
                <div class="form-group row">
